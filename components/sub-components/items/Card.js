...
const Card = ({ item }) => {
    ...
    return (
        <div className='w-1/4 bg-white max-w-full h-64 md:w-2/4 xl:w-4/12 rounded overflow-hidden m-4 mt-4 p-4 border-r border-l border-b border-darkGrey border-opacity-25 border-t hover:border-opacity-0 hover:shadow-lg transition ease-in-out duration-300 hover:-translate-y-1 transform'>
            ...
            <div className='mt-12 flex flex-wrap absolute bottom-0 mb-6'>
                {tech.map(item => (
                    <Badge item={item} />
                ))}
            </div>
